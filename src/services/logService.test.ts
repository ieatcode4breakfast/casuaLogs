...
describe('logService', () => {
  ...
  it('should save and retrieve a log decoupled from a template', async () => {
    ...
    await saveLog({ blocks: logBlocks as any });
  ...
  it('should enforce validation rules', async () => {
    await expect(saveLog({ blocks: [] })).rejects.toThrow('Log must contain at least one block.');

    const invalidHeader = [{ id: '1', type: 'header' as const, level: 1 as const, text: 'a'.repeat(51) }];
    await expect(saveLog({ blocks: invalidHeader as any })).rejects.toThrow('Header/Short-text blocks cannot exceed 50 characters.');

    const invalidTextLabel = [{ id: '1', type: 'text' as const, inputType: 'short' as const, label: 'a'.repeat(51), value: '' }];
    await expect(saveLog({ blocks: invalidTextLabel as any })).rejects.toThrow('Text block labels cannot exceed 50 characters.');

    const invalidParagraph = [{ id: '1', type: 'paragraph' as const, text: 'a'.repeat(5001) }];
    await expect(saveLog({ blocks: invalidParagraph as any })).rejects.toThrow('Paragraph blocks cannot exceed 5000 characters.');
  });

  it('should delete a log', async () => {
    const mockLogs = [{ id: 'test-log-id', createdAt: '2026-08-10', blocks: [] }];
