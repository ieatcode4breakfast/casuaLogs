...
export function Card({ title, onClick, color = 'blue', createdAt, updatedAt, leftFooterNode }: CardProps) {
  ...
  return (
    ...
    >
      <h3 className={`text-lg font-bold text-slate-900 dark:text-white transition-colors line-clamp-2 wrap-break-word ${hoverColorClass} ${(createdAt || updatedAt || leftFooterNode) ? 'mb-2' : ''}`}>
        {title}
      </h3>
